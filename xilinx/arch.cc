...
void Arch::routeClock()
{
    ...
    for (auto& net : nets) {
        ...
        if ((ni->driver.cell->type == id_BUFGCTRL || ni->driver.cell->type == id_BUFCE_BUFG_PS ||
            ...
            ni->driver.port == id("O"))
            is_global = true;
        else if (ni->driver.cell->type == id("PLLE2_ADV_PLLE2_ADV") && ni->users.entries() == 1 &&
                 ((*ni->users.begin()).cell->type == id_BUFGCTRL || (*ni->users.begin()).cell->type == id_BUFCE_BUFCE ||
                  (*ni->users.begin()).cell->type == id_BUFGCE_DIV_BUFGCE_DIV))
            is_global = true;
        else if (ni->users.entries() == 1 && (*ni->users.begin()).cell->type == id("PLLE2_ADV_PLLE2_ADV") &&
                 (*ni->users.begin()).port == id("CLKIN1"))
            is_global = true;
        if (!is_global)
            continue;
        ...
        for (auto &usr : ni->users) {
            ...
            if (getCtx()->debug)
                log_info("        routing arc to %s.%s (wire %s):\n", usr.cell->name.c_str(this), usr.port.c_str(this),
                         nameOfWire(getCtx()->getNetinfoSinkWire(ni, usr, 0)));
            visit.push(getCtx()->getNetinfoSinkWire(ni, usr, 0));
            while (!visit.empty()) {
                WireId curr = visit.front();
                visit.pop();
            ...
            if (dest == WireId()) {
                ...
                if (ni->users.entries() == 1 && (*ni->users.begin()).cell->type == id("PLLE2_ADV_PLLE2_ADV") &&
                    (*ni->users.begin()).port == id("CLKIN1")) {
